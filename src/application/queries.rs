use crate::infrastructure::pipeline::{
    ...
    transforms::{Aggregate, AggFunc, Filter, FilterOp, Join, JoinType, Pipeline, Select, Sort},
};
use crate::domain::{
    finance::TransactionRepository,
...
pub fn query_dept_spend_summary(
    ...
    let result = Pipeline::new("dept_spend_summary")
        ...
        .step(Sort { field: "total_spend".into(), descending: true })
        .run(transactions_dataset(tx_repo));

    result.records.iter().map(|r| {
        let f = |k: &str| r.get(k).and_then(Value::as_f64).unwrap_or(0.0);
        let i = |k: &str| match r.get(k) { Some(Value::Int(n)) => *n, _ => 0 };
...
pub fn query_top_earners(emp_repo: &dyn EmployeeRepository) -> Vec<TopEarner> {
    let result = Pipeline::new("top_earners")
        .step(Select {
            fields: vec!["name".into(), "department".into(), "level".into(), "salary".into()],
        })
        .step(Sort { field: "salary".into(), descending: true })
...
pub fn query_high_value_transactions(
    ...
    let result = Pipeline::new("high_value_txns")
        ...
        .step(Sort { field: "amount".into(), descending: true })
        .run(transactions_dataset(tx_repo));
