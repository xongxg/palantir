...
use crate::application::ontology::{
    bounded_context::ContextMap, ddd_mapping::DddMapping, graph::OntologyGraph,
...
impl JsonExporter {
    ...
    pub fn export(graph: &OntologyGraph, mapping: &DddMapping, context_map: &ContextMap) -> String {
        ...
        for (i, bc) in context_map.contexts.iter().enumerate() {
            ...
            out.push_str(&fmt_kv(
                "cohesion",
                &format!("{:.2}", bc.cohesion),
                3,
                false,
            ));
            out.push_str(&fmt_kv(
                "internal_links",
                &bc.internal_links.to_string(),
                3,
                false,
            ));
        ...
        out.push_str(&fmt_kv(
            "total_entities",
            &graph.objects.len().to_string(),
            3,
            false,
        ));
        out.push_str(&fmt_kv(
            "total_relationships",
            &graph.relationships.len().to_string(),
            3,
            false,
        ));
        out.push_str(&fmt_kv(
            "bounded_contexts",
            &context_map.contexts.len().to_string(),
            3,
            true,
        ));
        out.push_str("  }\n");
