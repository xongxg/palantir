...
fn db() -> &'static Db { DB.get().expect("DB not initialised") }

// ── Per-project in-memory state ───────────────────────────────────────────────

#[derive(Clone, Serialize, Deserialize)]
struct CsvConnector { id: String, path: String, ns: String, schema: String }
...
#[derive(Clone, Serialize)]
struct LiveRel { from: String, to: String, kind: String }

#[derive(Default)]
struct LiveGraph { entities: HashMap<String, LiveEntity>, relationships: Vec<LiveRel> }
...
async fn projects_page() -> Html<&'static str> { Html(include_str!("ui/projects.html")) }
async fn workspace_page() -> Html<&'static str> { Html(include_str!("ui/workspace.html")) }
async fn viz_page()       -> Html<&'static str> { Html(include_str!("../../../assets/index.html")) }
...
async fn list_project_connectors(Path(id): Path<String>) -> impl IntoResponse {
    ...
    if !has {
        if let Ok(rows) = db().load_connectors(&id).await {
            ...
            for c in rows {
                map.insert(c.id.clone(), CsvConnector {
                    id: c.id, path: c.path, ns: c.ns, schema: c.schema_name,
                });
            }
        }
    }
    ...
    match db().load_connectors(&id).await {
        Ok(rows) => {
            let list: Vec<serde_json::Value> = rows.iter().map(|c| {
                let headers: serde_json::Value = c.headers.as_deref()
                    .and_then(|s| serde_json::from_str(s).ok())
                    .unwrap_or(json!([]));
                let samples: serde_json::Value = c.samples.as_deref()
                    .and_then(|s| serde_json::from_str(s).ok())
                    .unwrap_or(json!([]));
                let mapping: serde_json::Value = c.mapping_config.as_deref()
                    .and_then(|s| serde_json::from_str(s).ok())
                    .unwrap_or(json!(null));
                json!({
...
fn read_csv_meta(path: &str) -> (String, String) {
    let Ok(mut rdr) = csv::Reader::from_path(path) else { return ("[]".into(), "[]".into()); };
    let Ok(hdr) = rdr.headers().cloned() else { return ("[]".into(), "[]".into()); };
    let headers: Vec<String> = hdr.iter().map(|s| s.to_string()).collect();
    let mut samples: Vec<serde_json::Value> = Vec::new();
    for rec in rdr.records().take(5).flatten() {
        let mut obj = serde_json::Map::new();
        for (k, v) in headers.iter().zip(rec.iter()) { obj.insert(k.clone(), json!(v)); }
...
async fn delete_connector(
    ...
    let project_id = match params.get("project_id") {
        Some(id) => id.clone(),
        None     => return (StatusCode::BAD_REQUEST, Json(json!({"error":"project_id required"}))).into_response(),
    };
    // Get file path before removing from memory
    let path = CONNECTORS.read().await
        .get(&project_id).and_then(|m| m.get(&connector_id)).map(|c| c.path.clone());

    // Remove from memory
    CONNECTORS.write().await
...
async fn save_connector_mapping(
    ...
    match db().save_connector_mapping(&connector_id, &config.to_string()).await {
        Ok(_)  => (StatusCode::OK, Json(json!({"ok": true}))).into_response(),
        Err(e) => (StatusCode::INTERNAL_SERVER_ERROR, Json(json!({"error": e.to_string()}))).into_response(),
    }
}
...
async fn upload_csv(
    ...
    match db().get_project(&project_id).await {
        Ok(None) => return (StatusCode::NOT_FOUND, Json(json!({"error":"project not found"}))).into_response(),
    ...
    if let Err(e) = tokio::fs::create_dir_all(&upload_dir).await {
        return (StatusCode::INTERNAL_SERVER_ERROR,
            Json(json!({"error":"create upload dir failed","detail": e.to_string()}))).into_response();
    }
            ...
            match field.bytes().await {
                Ok(bytes) => {
                    ...
                    if let Err(e) = tokio::fs::write(&dest, &bytes).await {
                        return (StatusCode::INTERNAL_SERVER_ERROR,
                            Json(json!({"error":"write failed","detail": e.to_string()}))).into_response();
                    }
                    ...
                    // DB
                    let _ = db().save_connector(&ConnectorRow {
                        id: id.clone(), project_id: project_id.clone(),
                        path: conn.path.clone(), ns: ns.clone(), schema_name: schema.clone(),
                ...
                Err(e) => return (StatusCode::BAD_REQUEST,
                    Json(json!({"error":"read multipart failed","detail": e.to_string()}))).into_response(),
            }
        }
    }
    match items.len() {
...
#[derive(Deserialize)]
struct InspectReq { connector_id: String, project_id: String, #[serde(default)] limit: Option<usize> }

async fn inspect(Json(req): Json<InspectReq>) -> impl IntoResponse {
    let c = match CONNECTORS.read().await
        .get(&req.project_id).and_then(|m| m.get(&req.connector_id)).cloned()
    {
        Some(c) => c,
        None    => return (StatusCode::NOT_FOUND,
            Json(json!({"error":"connector not found","id": req.connector_id}))).into_response(),
    };
    let mut rdr = match csv::Reader::from_path(&c.path) {
        Ok(r)  => r,
        Err(e) => return (StatusCode::BAD_REQUEST, Json(json!({"error":"open csv failed","detail": e.to_string()}))).into_response(),
    };
    let headers: Vec<String> = match rdr.headers() {
        Ok(h)  => h.iter().map(|s| s.to_string()).collect(),
        Err(e) => return (StatusCode::BAD_REQUEST, Json(json!({"error":"read headers failed","detail": e.to_string()}))).into_response(),
    };
    ...
    for (i, rec) in rdr.records().enumerate() {
        ...
        let rec = match rec {
            Ok(r)  => r,
            Err(e) => return (StatusCode::BAD_REQUEST,
                Json(json!({"error":"read record failed","detail": e.to_string(), "row": i}))).into_response(),
        };
        let mut obj = serde_json::Map::new();
        for (k, v) in headers.iter().zip(rec.iter()) { obj.insert(k.clone(), json!(v)); }
        samples.push(serde_json::Value::Object(obj));
    }
    (StatusCode::OK, Json(json!({"headers": headers, "samples": samples, "ns": c.ns, "schema": c.schema}))).into_response()
}
...
fn default_col_type() -> String { "str".into() }
...
#[derive(Deserialize)]
struct WorkspaceBuildReq { project_id: String, items: Vec<WorkspaceItem> }

fn build_workspace_toml(ns: &str, entity_type: &str, id_field: &str, cols: &[WorkspaceColConfig]) -> String {
    let map_lines: String = cols.iter()
...
async fn workspace_build(Json(req): Json<WorkspaceBuildReq>) -> impl IntoResponse {
    ...
    for item in &req.items {
        let c = match CONNECTORS.read().await
            .get(pid).and_then(|m| m.get(&item.connector_id)).cloned()
        {
            Some(c) => c,
            None    => return (StatusCode::NOT_FOUND,
                Json(json!({"error":"connector not found","id": item.connector_id}))).into_response(),
        };
        ...
        let mapping = match TomlMapping::from_str(&toml) {
            Ok(m)  => m,
            Err(e) => return (StatusCode::BAD_REQUEST, Json(json!({"error":"invalid mapping","detail": e.to_string()}))).into_response(),
        };
        ...
        while let Some(item_res) = stream.next().await {
            let rec = match item_res {
                Ok(r)  => r,
                Err(e) => return (StatusCode::BAD_REQUEST, Json(json!({"error":"read record failed","detail": e.to_string()}))).into_response(),
            };
            match mapping.apply(&rec, &schema) {
                Ok(ev) => all_events.extend(ev),
                Err(e) => return (StatusCode::BAD_REQUEST, Json(json!({"error":"mapping apply failed","detail": e.to_string()}))).into_response(),
            }
        }
    ...
    for item in &req.items {
        ...
        let _ = db().save_connector_mapping(&item.connector_id, &config.to_string()).await;
    }
    ...
    let _ = db().clear_project_graph(pid).await;
    {
        ...
        if let Some(g) = gs.get(pid) {
            for e in g.entities.values() {
                ...
                let _ = db().upsert_entity(&EntityRow {
                    id: e.id.clone(), project_id: pid.clone(),
                    entity_type: e.r#type.clone(), ddd_concept: e.ddd_concept.clone(),
                    label: e.label.clone(), properties: props,
                }).await;
            }
            for r in &g.relationships {
                let _ = db().upsert_relationship(&RelRow {
                    project_id: pid.clone(),
                    from_id: r.from.clone(), to_id: r.to.clone(), kind: r.kind.clone(),
                }).await;
            }
        }
    }
    ...
    let (entity_cnt, rel_cnt) = gs_snap.get(pid)
        .map(|g| (g.entities.len() as i64, g.relationships.len() as i64))
        .unwrap_or((0, 0));
    drop(gs_snap);
    let ents_snap: Vec<LiveEntity> = {
        let gs = LIVE_GRAPHS.read().await;
        gs.get(pid).map(|g| g.entities.values().cloned().collect()).unwrap_or_default()
    };
    let rels_snap: Vec<LiveRel> = {
        let gs = LIVE_GRAPHS.read().await;
        gs.get(pid).map(|g| g.relationships.clone()).unwrap_or_default()
    };
    let (bcs_snap, _) = compute_contexts(&ents_snap, &rels_snap);
    let _ = db().save_build(&BuildRow {
    ...
    let gs = LIVE_GRAPHS.read().await;
    let g  = gs.get(pid).map(|g| (g.entities.len(), g.relationships.len())).unwrap_or((0, 0));
    drop(gs);
    ...
    for ev in events {
        match ev {
            OntologyEvent::Upsert { object } => {
                ...
                for (k, v) in object.attrs.iter() { props.insert(k.clone(), value_to_json(v)); }
                let label = props.get("name").and_then(|v| v.as_str()).unwrap_or(&object.id.0).to_string();
                g.entities.insert(object.id.0.clone(), LiveEntity {
                    id: object.id.0, r#type: object.entity_type,
                    ddd_concept: "Entity".into(), label, properties: props,
                });
            }
            OntologyEvent::Delete { id } => { g.entities.remove(&id.0); }
            OntologyEvent::Link { from, to, rel, .. } => {
                g.relationships.push(LiveRel { from: from.0, to: to.0, kind: rel });
            }
        }
    }
        ...
        if let Ok(entity_rows) = db().load_entities(&pid).await {
            if let Ok(rel_rows) = db().load_relationships(&pid).await {
                ...
                for e in entity_rows {
                    ...
                    g.entities.insert(e.id.clone(), LiveEntity {
                        id: e.id, r#type: e.entity_type, ddd_concept: e.ddd_concept,
                        label: e.label, properties: props,
                    });
                }
                for r in rel_rows {
                    g.relationships.push(LiveRel { from: r.from_id, to: r.to_id, kind: r.kind });
                }
            }
        }
...
#[derive(Deserialize)]
struct ResetReq { project_id: String }
...
fn title_case(s: &str) -> String {
    ...
    let mut cap = true;
    for c in s.chars() {
        if !c.is_alphanumeric() { cap = true; continue; }
        if cap { out.push(c.to_ascii_uppercase()); cap = false; } else { out.push(c); }
    }
    ...
    for (eid, ent) in g.entities.iter() {
        for (k, v) in ent.properties.iter() {
            if let Some(s) = v.as_str() {
                if !s.is_empty() { occ.entry((k.clone(), s.to_string())).or_default().push(eid.clone()); }
            }
        }
    }
    let mut existing: HashSet<(String, String, String)> = g.relationships.iter()
        .map(|r| (r.from.clone(), r.to.clone(), r.kind.clone())).collect();
    for ((k, val), ids) in occ {
        if ids.len() < 2 { continue; }
        let group_id = format!("dim::{}::{}", k, val);
        g.entities.entry(group_id.clone()).or_insert_with(|| LiveEntity {
            id: group_id.clone(), r#type: title_case(&k),
            ddd_concept: "Value Object".into(), label: val.clone(),
            properties: serde_json::Map::new(),
        });
        for e in ids {
            let rel = (e.clone(), group_id.clone(), "BELONGS_TO".into());
            if !existing.contains(&rel) {
                g.relationships.push(LiveRel { from: e, to: group_id.clone(), kind: "BELONGS_TO".into() });
                existing.insert(rel);
            }
        }
    }
...
fn compute_contexts(entities: &[LiveEntity], relationships: &[LiveRel]) -> (Vec<serde_json::Value>, Vec<serde_json::Value>) {
    let mut types: HashMap<String, usize> = HashMap::new();
    for e in entities { if e.ddd_concept != "Value Object" { types.entry(e.r#type.clone()).or_insert(0); } }
    let bcs: Vec<_> = types.keys()
        .map(|t| json!({"name": t, "entity_types": [t], "internal_links": 0, "cohesion": 0.0}))
        .collect();
    let id_type: HashMap<_, _> = entities.iter().map(|e| (e.id.clone(), e.r#type.clone())).collect();
    let mut agg: HashMap<(String, String, String), usize> = HashMap::new();
    for r in relationships {
        if r.kind != "HAS" { continue; }
        if let (Some(a), Some(b)) = (id_type.get(&r.from), id_type.get(&r.to)) {
            if a != b { *agg.entry((a.clone(), b.clone(), "HAS".into())).or_insert(0) += 1; }
        }
    }
            ...
            if k.ends_with("_id") {
                if let Some(s) = v.as_str() {
                    if ids.contains(s) {
                        ...
                        if !existing.contains(&rel) {
                            g.relationships.push(LiveRel { from: s.to_string(), to: eid.clone(), kind: "HAS".into() });
                            existing.insert(rel);
                        }
                    }
                }
...
async fn main() -> anyhow::Result<()> {
    ...
    let app = Router::new()
        ...
        .route("/api/projects/:id",            get(get_project).delete(delete_project_handler))
        .route("/api/projects/:id/connectors", get(list_project_connectors))
        .route("/api/projects/:id/builds",     get(list_project_builds))
        // Connectors
        .route("/api/connectors/:id",      axum::routing::delete(delete_connector).post(save_connector_mapping))
        // Ingest
        .route("/api/upload",              post(upload_csv))
        .route("/api/inspect",             post(inspect))
        .route("/api/workspace/build",     post(workspace_build))
        .route("/api/discover",            post(|| async { (StatusCode::OK, Json(json!({"ok":true}))) }))
        .route("/api/live_ontology",       get(live_ontology))
        .route("/api/reset",               post(reset))
        .nest_service("/static", ServeDir::new("crates/palantir-ingest-api/src/ui/static"))
        .nest_service("/docs",   ServeDir::new("docs"));

    let addr_str = std::env::args().nth(1)
