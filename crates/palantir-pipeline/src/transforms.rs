...
impl Pipeline {
    pub fn new(name: &str) -> Self { Self { name: name.into(), steps: Vec::new() } }
...
#[derive(Debug, Clone)]
pub enum FilterOp { Eq, Ne, Gt, Lt, Ge, Le, Contains }
...
#[derive(Debug, Clone)]
pub struct Select { pub fields: Vec<String> }
    ...
    fn run(&self, input: Dataset) -> Dataset {
        ...
        for rec in input.records {
            ...
            for f in &self.fields {
                if let Some(v) = rec.get(f) { m.insert(f.clone(), v.clone()); }
            }
            out.push(Record { id: rec.id, fields: m });
        }
        out
    }
}
...
#[derive(Debug, Clone)]
pub enum AggFunc { Sum(String), Count, Avg(String), Max(String), Min(String) }

#[derive(Debug, Clone)]
pub struct Aggregate { pub group_by: Vec<String>, pub aggregations: Vec<(String, AggFunc)> }

impl Transform for Aggregate {
    fn name(&self) -> &str { "aggregate" }
    fn run(&self, input: Dataset) -> Dataset {
        #[derive(Default)]
        struct Acc { count: i64, sums: HashMap<String, f64>, max: HashMap<String, f64>, min: HashMap<String, f64> }

        let mut groups: HashMap<Vec<String>, Acc> = HashMap::new();
        for r in &input.records {
            let key: Vec<String> = self.group_by.iter().map(|f| r.get(f).map(to_string).unwrap_or_default()).collect();
            ...
            for (_, func) in &self.aggregations {
                match func {
                    ...
                    AggFunc::Max(field) => {
                        let v = r.get(field).and_then(Value::as_f64).unwrap_or(f64::NEG_INFINITY);
                        let e = ent.max.entry(field.clone()).or_insert(f64::NEG_INFINITY);
                        if v > *e { *e = v; }
                    }
                    AggFunc::Min(field) => {
                        let v = r.get(field).and_then(Value::as_f64).unwrap_or(f64::INFINITY);
            ...
            for (alias, func) in &self.aggregations {
                let v = match func {
                    ...
                    AggFunc::Avg(field) => {
                        ...
                        Value::Float(if acc.count == 0 { 0.0 } else { s / acc.count as f64 })
                    }
                    AggFunc::Max(field) => Value::Float(*acc.max.get(field).unwrap_or(&f64::NEG_INFINITY)),
                    AggFunc::Min(field) => Value::Float(*acc.min.get(field).unwrap_or(&f64::INFINITY)),
                };
                fields.insert(alias.clone(), v);
            }
            out.push(Record { id: uuid_like(), fields });
        }
        out
    }
}
...
#[derive(Debug, Clone)]
pub struct Sort { pub field: String, pub descending: bool }
...
#[derive(Debug, Clone)]
pub enum JoinType { Inner }

#[derive(Debug, Clone)]
pub struct Join { pub right: Dataset, pub left_key: String, pub right_key: String, pub join_type: JoinType }
...
impl Transform for Join {
    ...
    fn run(&self, input: Dataset) -> Dataset {
        ...
        for r in &self.right.records {
            if let Some(v) = r.get(&self.right_key) { idx.insert(to_string(v), r); }
        }
        let mut out = Dataset::new(&input.name, &input.object_type);
            ...
            if let Some(lk) = l.get(&self.left_key) {
                if let Some(rr) = idx.get(&to_string(lk)) {
                    ...
                    for (k,v) in rr.fields.iter() {
                        let key = if merged.contains_key(k) { format!("{}_r", k) } else { k.clone() };
                        merged.insert(key, v.clone());
                    }
                    out.push(Record { id: l.id.clone(), fields: merged });
                }
            }
        }
        out
...
fn to_string(v: &Value) -> String { match v { Value::Int(n) => n.to_string(), Value::Float(f) => f.to_string(), Value::Bool(b) => b.to_string(), Value::String(s) => s.clone(), Value::Json(j) => j.to_string(), Value::Null => String::new() } }
...
fn cmp_f(a: f64, b: f64, op: &FilterOp) -> bool { match op { FilterOp::Eq => a==b, FilterOp::Ne => a!=b, FilterOp::Gt => a>b, FilterOp::Lt => a<b, FilterOp::Ge => a>=b, FilterOp::Le => a<=b, FilterOp::Contains => false } }
fn cmp_s(a: &str, b: &str, op: &FilterOp) -> bool { match op { FilterOp::Eq => a==b, FilterOp::Ne => a!=b, FilterOp::Gt => a>b, FilterOp::Lt => a<b, FilterOp::Ge => a>=b, FilterOp::Le => a<=b, FilterOp::Contains => a.contains(b) } }
...
fn uuid_like() -> String { use std::time::{SystemTime, UNIX_EPOCH}; let ns = SystemTime::now().duration_since(UNIX_EPOCH).unwrap().as_nanos(); format!("r{:x}", ns) }
