...
//! PostgresStore — stub adapter, not yet implemented.
//! Set PALANTIR_DB=postgres://... and implement this module to activate.
    ...
    pub async fn connect(_url: &str) -> Result<Self> {
        anyhow::bail!(
            "PostgresStore is not yet implemented. \
             Contributions welcome — implement crates/palantir-db/src/adapters/postgres.rs"
        )
    }
}
