#![cfg(feature = "sqlite")]
//! SqliteStore — stub. The real SQLite implementation lives in `palantir-sqlite::Db`.
//! Use `palantir_sqlite::Db::open(path)` for the fully-featured SQLite backend.
...
impl SqliteStore {
    pub async fn open(_path: &str) -> Result<Self> {
        anyhow::bail!(
            "SqliteStore in palantir-db is a stub. \
             Use palantir_sqlite::Db::open(path) for the SQLite backend."
        )
    }
}

macro_rules! ni {
    () => { anyhow::bail!("SqliteStore: use palantir_sqlite::Db directly") };
}
