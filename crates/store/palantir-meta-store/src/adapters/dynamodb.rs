...
//! DynamoDbStore — stub adapter, not yet implemented.
//! Set PALANTIR_DB=dynamodb://... and implement this module to activate.
...
impl DynamoDbStore {
    pub async fn new(_region: &str, _prefix: &str, _endpoint: Option<&str>) -> Result<Self> {
        anyhow::bail!(
            "DynamoDbStore is not yet implemented. \
             Contributions welcome — implement crates/palantir-db/src/adapters/dynamodb.rs"
        )
    }
}

macro_rules! ni {
    () => { anyhow::bail!("DynamoDbStore: not yet implemented") };
}

#[async_trait]
impl MetadataStore for DynamoDbStore {
