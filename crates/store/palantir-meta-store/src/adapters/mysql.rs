#![cfg(feature = "mysql")]
//! MySqlStore — stub adapter, not yet implemented.
//! Set PALANTIR_DB=mysql://... and implement this module to activate.
...
impl MySqlStore {
    pub async fn connect(_url: &str) -> Result<Self> {
        anyhow::bail!(
            "MySqlStore is not yet implemented. \
             Contributions welcome — implement crates/palantir-db/src/adapters/mysql.rs"
        )
    }
}

macro_rules! ni {
    () => { anyhow::bail!("MySqlStore: not yet implemented") };
}
