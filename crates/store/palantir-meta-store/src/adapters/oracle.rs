#![cfg(feature = "oracle")]
//! OracleStore — stub adapter, not yet implemented.
//! Set PALANTIR_DB=oracle://... and implement this module to activate.
...
impl OracleStore {
    pub async fn connect(_url: &str) -> Result<Self> {
        anyhow::bail!(
            "OracleStore is not yet implemented. \
             Contributions welcome — implement crates/palantir-db/src/adapters/oracle.rs"
        )
    }
}

macro_rules! ni {
    () => { anyhow::bail!("OracleStore: not yet implemented") };
}

#[async_trait]
impl MetadataStore for OracleStore {
