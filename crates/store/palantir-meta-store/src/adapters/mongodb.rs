#![cfg(feature = "mongodb")]
//! MongoStore — stub adapter, not yet implemented.
//! Set PALANTIR_DB=mongodb://... and implement this module to activate.
...
impl MongoStore {
    pub async fn connect(_url: &str, _db: &str) -> Result<Self> {
        anyhow::bail!(
            "MongoStore is not yet implemented. \
             Contributions welcome — implement crates/palantir-db/src/adapters/mongodb.rs"
        )
    }
}

macro_rules! ni {
    () => { anyhow::bail!("MongoStore: not yet implemented") };
}
