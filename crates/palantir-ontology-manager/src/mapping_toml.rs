...
use crate::model::{
    CanonicalRecord, OntologyEvent, OntologyId, OntologyObject, OntologySchema, Provenance,
    TimeBounds, Value,
};
...
struct LinkSpec {
    ...
    to_entity: String,
    #[serde(default)]
    from_key: String, // if empty → current object's id
    #[serde(default)]
    to_key: String, // if empty → current object's id
}
...
impl TomlMapping {
    pub fn from_str(s: &str) -> Result<Self, MappingError> {
        let doc: MappingDoc =
            toml::from_str(s).map_err(|e| MappingError::Message(e.to_string()))?;
        Ok(Self { doc })
    }
...
impl Mapping for TomlMapping {
    ...
    fn apply(
        &self,
        rec: &CanonicalRecord,
        _schema: &OntologySchema,
    ) -> Result<Vec<OntologyEvent>, MappingError> {
        if !self.doc.from.ns.is_empty() && self.doc.from.ns != rec.ns {
            return Ok(vec![]);
        }
        ...
        let id_field = if self.doc.id.field.is_empty() {
            "id"
        } else {
            &self.doc.id.field
        };
        ...
        let object = OntologyObject {
            ...
            time: TimeBounds {
                valid_from: None,
                valid_to: None,
                tx_time: OffsetDateTime::now_utc(),
            },
            version: 1,
        ...
        let mut out = vec![OntologyEvent::Upsert {
            object: object.clone(),
        }];
        for l in &self.doc.links {
            let from_id = if l.from_key.is_empty() {
                object.id.0.clone()
            } else {
                rec.payload
                    .get(&l.from_key)
            ...
            let to_id = if l.to_key.is_empty() {
                object.id.0.clone()
            } else {
                rec.payload
                    .get(&l.to_key)
