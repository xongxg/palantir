...
pub fn parse_ts(s: &str) -> Option<i64> {
    ...
    if s.chars().nth(4) != Some('-') || s.chars().nth(7) != Some('-') { return None; }
    ...
    let mut days: i64 = (year - 1970) * 365 + (year - 1969) / 4
...
pub fn detect_ts_fields(dataset: &Dataset) -> Vec<String> {
    if dataset.records.is_empty() { return vec![]; }

    // Collect candidate field names (ends with temporal suffix)
    let mut candidates: Vec<String> = dataset.records.first()
        .map(|r| {
            let mut names: Vec<_> = r.fields.keys()
                .filter(|k| TS_SUFFIXES.iter().any(|s| k.ends_with(s))
    ...
    candidates.retain(|field| {
        ...
        for rec in &dataset.records {
            if let Some(Value::String(s)) = rec.get(field) {
                if !s.is_empty() { total += 1; }
                if parse_ts(s).is_some() { ok += 1; }
            }
        }
        ...
        if !ts_a.is_empty() && !ts_b.is_empty() && ts_a > ts_b {
            ...
            if violations.len() < 4 {
                violations.push(Violation {
                    row_a: i + 1, id_a: a.id.clone(), ts_a: ts_a.to_string(),
                    row_b: i + 2, id_b: b.id.clone(), ts_b: ts_b.to_string(),
                });
            }
        }
    }
...
pub fn compute_stats(dataset: &Dataset, field: &str) -> Option<TimelineStats> {
    ...
    let mut pts: Vec<(i64, String, String)> = dataset.records.iter()
        .filter_map(|r| {
    ...
    let intervals: Vec<i64> = pts.windows(2)
        .map(|w| w[1].0 - w[0].0)
        .collect();
...
pub fn detect_gaps(dataset: &Dataset, field: &str, min_gap_secs: i64) -> Vec<Gap> {
    let pts: Vec<_> = dataset.records.iter()
        .filter_map(|r| {
            let ts_str = r.get(field).and_then(Value::as_str)?;
            let secs   = parse_ts(ts_str)?;
...
pub fn daily_histogram(dataset: &Dataset, field: &str) -> Vec<DayBucket> {
    ...
    let mut buckets: Vec<DayBucket> = map.into_iter()
        .map(|(day, ids)| DayBucket { count: ids.len(), day, ids })
        .collect();
    ...
    for ev in &events.records {
        if let Some(fk_val) = ev.get(join_field).map(|v| v.to_string()) {
            if let Some(Value::String(ets)) = ev.get(event_ts_field) {
                event_index.entry(fk_val).or_insert_with(|| (&ev.id, ets.clone()));
            }
        }
    }

    let mut rows: Vec<LatencyEntry> = anchor.records.iter()
        .filter_map(|anc| {
