# Palantir Architecture — v0.1.3

> 版本：v0.1.3 | 日期：2026-03-19 | 状态：设计阶段
>
> 更新规则：每日 refine → patch 版本递增；服务新增/删除 → minor；底层存储/协议更换 → major
>
> **v0.1.3 变更**：新增 ADR-14 三层计算模型（L1 Redis / L2 本地内存 / L3 SurrealDB）；新增 ADR-15 事件序列按 EntityType 独立（NATS Subject 层级 + SurrealDB 复合主键）
...
| 14 | 计算分层模型 | L1 Redis（集中共享）/ L2 本地内存（服务私有）/ L3 SurrealDB（持久化）| ✅ |
...
```
L1：集中内存（Redis，跨服务共享）     延迟 < 1ms   容量受限   一致性：强
L2：本地内存（DashMap + petgraph）   延迟 μs 级   容量受限   一致性：最终
L3：磁盘（SurrealDB + RustFS）       延迟 ms 级   容量无限   一致性：强
```
...
| 层 | 存放内容 |
|----|---------|
| L1 Redis | Semantic Cache、ReBAC 授权结果、Workflow 冷却窗口、Agent Memory 热数据（TTL 72h）、分布式锁 |
| L2 本地内存 | function-svc：DashMap<OntologyId, OntologyObject> + petgraph 图；agent-svc：近期对象快照；auth-svc：Policy 规则集；workflow-svc：活跃 Workflow 状态机 |
| L3 SurrealDB | 全量 Ontology（历史版本）、事件日志、冷查询、L2 宕机重建来源 |
...
请求 → L2 命中？→ 返回（μs）
      ...
      L1 命中？→ 返回（< 1ms），顺手暖 L2
      ↓ miss
      L3 查询 → 返回（ms），顺手暖 L1 + L2
```

**一致性机制**：写路径只走 L3，ontology-svc 写入后发布 OntologyEvent → L2 各服务 patch 本地投影，L1 相关 key 失效。
